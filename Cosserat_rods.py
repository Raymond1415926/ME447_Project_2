...
class CosseratRod():
    def __init__(self, number_of_elements, total_length, density, radius, direction, normal, youngs_modulus, dt,
                 total_time, poisson_ratio=0.5, shear_modulus=0):
        ...
        self.reference_lengths = np.ones(
            (self.n_elements)) * self.total_length / self.n_elements  # Here I took total rod length as 1
        ...
        self.position = np.zeros((3, self.n_nodes))
        ...
        self.external_torque = np.zeros([1, self.n_nodes])
    ...
    def force_rule(self, position):
        ...
        self.tangents = self.position[:, 1:] - self.position[:, :-1]
        self.norm_lengths = np.linalg.norm(self.tangents, axis=0, keepdims=True)
        ...
        self.external_torque = np.ones([3, self.n_elements])
        ...
        for element in range(n_elements):
            angular_acceleration[:, element] = (diff_B_k_over_epsilon_3[:, element] + diff_k_cross_B_over_epsilon_3[:, element] + Q_t_cross_S_sigma_l[:, element] \
                                    + self.external_torque[:, element]) / J_over_e[:, element]
    ...
    def position_verlet(self, dt, x, v):
        ...
        # update Q
        self.Q = np.exp(-dt / 2 * half_step_angular_acceleration) * Q_t_half_dt
